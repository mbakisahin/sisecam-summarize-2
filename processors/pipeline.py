...
class PipelineCoordinator:
    ...
    def process_pdf(self, blob_name, file_name, file_content, system_message_summarization, system_message_final):
        """
        ...
        embedding_data = self.embedding_pipeline.process_summary(final_summary, full_file_name)

        if embedding_data:
            self._find_and_compare_nearest_neighbors(embedding_data, blob_name.split('/')[1],
                                                                         final_summary, full_file_name)
            self.indexer.ingest_embeddings([embedding_data], final_summary)

    def _find_and_compare_nearest_neighbors(self, embedding_data, keyword, final_summary, full_file_name):
        """
